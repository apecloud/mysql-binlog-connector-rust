mod test {
    ...
    use crate::test::{
        ...
        test_common::test::{TestCommon, D_EVENTS},
    };
    ...
    fn test_delete_multiple_rows() {
        ...
        TestCommon::execute_delete_sqls_and_get_binlogs(vec![], vec![]);

        unsafe {
            assert_eq!(D_EVENTS.len(), 1);
            assert_eq!(D_EVENTS[0].rows.len(), 5);
            ...
            // NULL fields
            for i in 0..13 {
                assert!(&D_EVENTS[0].rows[3].column_values[2 * i + 1].is_none());
            }
            // non-Null fields
            for i in 0..13 {
                assert!(&D_EVENTS[0].rows[3].column_values[2 * i].is_some());
            }
            DmlTestCommon::check_values(
                &D_EVENTS[0].rows[3],
                11,
            ...
            for i in 0..27 {
                assert!(&D_EVENTS[0].rows[4].column_values[i].is_none());
            }
        }
    }
}
